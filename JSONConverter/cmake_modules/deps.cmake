...
macro(add_dependency_module dir name)
	...
	elseif("${ARGV2}" STREQUAL "EXECUTE_SEPARATELY")
		...
		module_message("${name} variables before adding the directory: ${name}_SOURCE_DIR=${${name}_SOURCE_DIR}, ${name}_BINARY_DIR=${${name}_BINARY_DIR}, ${name}_INCLUDES=${${name}_INCLUDES}")
		if(NOT ${name}_POPULATED)
			module_message("FetchContent_Populate(${name})")
			FetchContent_Populate(${name})
			add_subdirectory(${dir} ${dir}/${BUILD_DIR_NAME})
			module_message("${name} variables after adding directory: ${name}_SOURCE_DIR=${${name}_SOURCE_DIR}, ${name}_BINARY_DIR=${${name}_BINARY_DIR}, ${name}_INCLUDES=${${name}_INCLUDES}")
		else()
			get_directory_property(${name}_INCLUDES DIRECTORY ${${name}_SOURCE_DIR} DEFINITION ${name}_INCLUDES)
			module_message("${name} variables after fetching: ${name}_SOURCE_DIR=${${name}_SOURCE_DIR}, ${name}_BINARY_DIR=${${name}_BINARY_DIR}, ${name}_INCLUDES=${${name}_INCLUDES}")
		endif()
	endif()
